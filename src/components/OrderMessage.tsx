...
const OrderMessage = ({navigation , orderList , deliveryDate, totalGroupOrderAmount, deliveryStatus , userDetails} : any) => {
  ...
  return (
    <View style={styles.OrderMessageContainer}>
        <Text style={styles.DeliveryDateHeading}> <Text style={{fontSize : FONTSIZE.size_16 , color : COLORS.primaryBlackHex}}>Delivered {deliveryStatus==='Delivered' ? "On" : "By"} - </Text>{deliveryDate}</Text>
      ...
      {orderList.length >0 && orderList.map((order:any , index : any) => {
        return (
            <View style={{marginTop : SPACING.space_10}} key={index}>
                ...
                <Text style={styles.UserHeadingText}>{index+1}. {userDetails[order.userId]} {loginUserId===order.userId ? "(You)" : ""}</Text>
