...
const OrderMessage = ({navigation , orderList , deliveryDate, totalGroupOrderAmount, deliveryStatus , userDetails} : any) => {
  ...
  return (
    <View style={styles.OrderMessageContainer}>
        <Text style={styles.DeliveryDateHeading}> <Text style={{fontSize : FONTSIZE.size_16 , color : COLORS.primaryBlackHex}}>{t("delivered")} {deliveryStatus==='Delivered' ? t("On") : t("By")} - </Text>{moment(deliveryDate).format("DD MMMM, YYYY")}</Text>
        <Text style={styles.DeliveryDateHeading}> <Text style={{fontSize : FONTSIZE.size_16 , color : COLORS.primaryBlackHex}}>{t("Total Amount")} - </Text>{t("Rs.")} {totalGroupOrderAmount}</Text>
        <Text style={styles.DeliveryDateHeading}> <Text style={{fontSize : FONTSIZE.size_16 , color : COLORS.primaryBlackHex}}>{t("Status")} - </Text>{t(deliveryStatus)}</Text>
      {orderList.length >0 && orderList.map((order:any , index : any) => {
        return (
            <View style={{marginTop : SPACING.space_10}} key={index}>
                ...
                <Text style={styles.UserHeadingText}>{index+1}. {userDetails[order.userId]!== undefined && userDetails[order.userId].name} ({ userDetails[order.userId]!== undefined && userDetails[order.userId].phone}) {loginUserId===order.userId ? `(${t("You")})` : ""}</Text>
