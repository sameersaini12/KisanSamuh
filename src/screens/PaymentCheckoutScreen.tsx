...
const PaymentCheckoutScreen = ({ navigation, route }: any) => {
  ...
  const findDeliveryDate = async () => {
    if (buyingGroupName === undefined) {
      ...
      var finalDate = moment(nextDeliveryDate).format("DD MMM, YYYY")
    ...
    <GestureHandlerRootView>
      ...
      <View style={styles.PaymentScreenHeader}>
        <View style={styles.PaymentScreenHeaderLeft}>
          ...
          <Text style={styles.PaymentScreenHeaderTitle}>{t("Payment")}</Text>
        </View>
      </View>
      ...
      >
        <Text style={styles.PaymentMethodListHeading}>{t("Total Order Price")}</Text>
        ...
        <View style={styles.ItemPriceContainer}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: SPACING.space_10 * 0.4 }}>
            ...
            <Text style={styles.ItemPriceText}>{t("Rs.")} {route.params.totalCartPrice}</Text>
          </View>
          {/* <View style={{flexDirection : "row" , justifyContent : "space-between" , alignItems : "center" ,marginBottom: SPACING.space_10*0.4}}>
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: SPACING.space_10 * 0.4 }}>
            <Text style={styles.ItemPriceHeading}>{t("Delivery")}</Text>
            <Text style={styles.ItemPriceText}>{t("Rs.")} {deliveryCost}</Text>
          </View>
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: SPACING.space_10 }}>
            <Text style={styles.ItemPriceHeading}>{t("Total Amount")}</Text>
            <Text style={styles.ItemPriceText}>{t("Rs.")} {Number(route.params.totalCartPrice) + Number(deliveryCost)}</Text>
          </View>
          {Math.floor(route.params.totalCartPrice / 10) !== 0 &&
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              <Text style={styles.ItemPriceHeading}>{t("You will get")}</Text>
        ...
        <Text style={styles.PaymentMethodListHeading}>{t("Delivery Address")}</Text>
        ...
        <View style={[styles.DeliveryAddressContainer, { marginTop: -SPACING.space_10 }]}>
          <View style={{ flexDirection: "row" }}>
            <Text style={[styles.DeliveryAddressText, { marginLeft: SPACING.space_10 * 0.3 }]}> <Text style={{ fontFamily: FONTFAMILY.poppins_semibold }}>{t("Delivered By")} - </Text>{deliveryDate}</Text>
          </View>
        </View>

        <Text style={styles.PaymentMethodListHeading}>{t("Choose your Payment Method")}</Text>
        ...
        <View style={styles.PaymentMethodListContainer}>
          ...
          >
            <View style={styles.PaymentMethodRadioContainer}>
              ...
              <Text style={styles.AddressListItemText}>{t(paymentMethodList[1])}</Text>
            </View>
            <Pressable
              onPress={() => cashOnDeliveryButtonHandler()}
              style={[styles.PlaceOrderButtonContainer, { display: selectedPaymentMethod == 1 ? "flex" : "none" }]}
            >
              <Text style={styles.PlaceOrderButtonText}>{t("Place Your Order")}</Text>
            </Pressable>
          </Pressable>
        </View>
      </ScrollView>
